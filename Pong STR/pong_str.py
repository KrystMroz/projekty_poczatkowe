...
paletka1.fill(BLUE)
# ustawienie prostkąta zawierającego paletkę w początkowej pozycji
...
P_SZER = 20
P_WYS = 20
P_PREDKOSC_X = 6 # predkość pozioma x
P_PREDKOSC_Y = 6 # predkość pionowa y
GREEN = (0, 255, 0) # kolor piłki
# utworzenie powierzchni piłki, narysowanie piłki i wypełnieniem kolorem
...
def drukuj_punktyAI():
    ...
    teskst_prostAI = tekstAI.get_rect()
    teskst_prostAI.center = (OKNOGRY_SZER / 2, OKNOGRY_WYS / 4)
    oknogry.blit(tekstAI, teskst_prostAI)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == MOUSEMOTION:
            ...
            # jeżeli wykraczamy pozna okno gry w lewo
            if przesuniecie < 0:
                przesuniecie = 0
    ...
    if pilka_prost.top <= 0: # piłka uciekła górą
       # P_PREDKOSC_Y *= -1 # odwracamy kierunek ruchu pionowego piłki
        pilka_prost.x = OKNOGRY_SZER / 2 # więc startuje ze środka
        pilka_prost.y = OKNOGRY_WYS / 2
    ...
    #jeżeli piłka dotknie paletki gracza, skieruj ją w przeciwną strone
    if pilka_prost.colliderect(paletka1_prost):
        P_PREDKOSC_Y *= -1
        # zapogiegaj przysłanianiu paletki przez piłkę
